...
chn = sample(model1(data), NUTS(1000, 0.85), 2000)
μ_nuts = describe(chn)[1][:, :mean]
σ_nuts = describe(chn)[1][:, :std]

@test all(isapprox.(rhat, fill(1.0, 2), atol = 0.05))
@test all(isapprox.(μ_nuts, μ_de, atol = 0.01))
@test all(isapprox.(σ_nuts, σ_de, atol = 0.01))
