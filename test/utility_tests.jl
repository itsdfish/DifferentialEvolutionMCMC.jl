@testset verbose = true "utility tests" begin
    ...
    @safetestset "reset!" begin
        ...
        p1 = Particle(Θ = [[0.7, 0.5, 0.1], 0.4, 0.6])
        p2 = Particle(Θ = [[0.9, 0.8, 0.5], 0.7, 0.8])
        ...
        p1 = Particle(Θ = [[0.7 0.5; 0.1 0.3], 0.4, 0.6])
        p2 = Particle(Θ = [[0.9 0.8; 0.5 0.2], 0.7, 0.8])
    ...
    @safetestset "Migration" begin
        using DifferentialEvolutionMCMC, Test, Random, Parameters, Distributions
        import DifferentialEvolutionMCMC:
            select_groups, select_particles, shift_particles!, sample_init
    ...
    @safetestset "particle operations" begin
        ...
        p1 = Particle(Θ = [1.0, 2.0])
        p2 = Particle(Θ = [-2.0, 3.0])
        pr = 3 * (p1 - p2)
        @test pr.Θ ≈ [9, -3]

        p1 = Particle(Θ = [1.0, 2.0])
        p2 = Particle(Θ = [-2.0, 3.0])
        p3 = Particle(Θ = [-2.0, 3.0])
        pr = 3 * (p1 - p2) + p3
        @test pr.Θ ≈ [7, 0]
        ...
        b = Uniform(-0.1, 0.1)
        pr = p1 + b
        @test p1.Θ ≈ pr.Θ atol = 0.2 # cummulative error
        @test p1.Θ ≠ pr.Θ
    end
end
